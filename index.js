...
const { color, bgcolor } = require('./ISALL-V3/color')
const { help } = require('./ISALL-V3/help')
const { donasi } = require('./ISALL-V3/donasi')
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./ISALL-V3/functions')
const { fetchJson } = require('./ISALL-V3/fetcher')
const { recognize } = require('./ISALL-V3/ocr')
const fs = require('fs')
...
const welkom = JSON.parse(fs.readFileSync('./ISALL-V3/welkom.json'))
const nsfw = JSON.parse(fs.readFileSync('./ISALL-V3/nsfw.json'))
const samih = JSON.parse(fs.readFileSync('./ISALL-V3/simi.json'))
const vcard = 'BEGIN:VCARD\n' // OM JANGAN DI UBAH OM
            + 'VERSION:4.0\n' // INI WEEM RAMLAN GANS
            + 'FN:ISALL GANS\n' // UBAH INI
            + 'ORG:Owner BOT;\n' // UBAH INI
            + 'TEL;type=CELL;type=VOICE;waid=6289630267618:+62 812-8203-2257\n' // UBAH INI
...
async function starts() {
  ...
  for (i = 0; i < 3; i++) {
    console.log(`[ ${moment().format("HH:mm:ss")} ] => Hallo Gw Isall :)`)
  }
}, 15000)
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SELAMAT TINGGAL... @${num.split('@')[0]}👋* \n_Kalo balik nitip gorengan um ͡° ͜ʖ ͡°_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6281282032257@s.whatsapp.net"] // Ganti pake nomor lu
			const ISALLNumber = ["6289630267618@s.whatsapp.net"] // JANGAN DI UBAH NGENTOD
			...
			const isISALLGANS = ISALLNumber.includes(sender)
			const isUrl = (url) => {
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Daftar Orang Goblok di kontak :\n'
               ...
               client.sendMessage(from, 'Nih nomor GW ANJING😍, jangan di galakin ya:*',MessageType.text, { quoted: mek} )
                break
				case 'ocr':
				...
				case 'tts':
					...
					const gtts = require('./ISALL-V3/gtts')(args[0])
				...
				case 'hilih':
					...
					break
				case 'ytmp3':
					if (args.length < 1) return reply('Urlnya mana TOLOL?')
					...
					break
				case 'ytstalk':
					if (args.length < 1) return reply('Yang mau di cari apaan? MEKI ATAU TITIT?')
					...
					break
				case 'tiktod':
					if (args.length < 1) return reply('Linknya mana TOLOL?')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.push(from)
						fs.writeFileSync('./ISALL-V3/simi.json', JSON.stringify(samih))
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						welkom.push(from)
						fs.writeFileSync('./ISALL-V3/welkom.json', JSON.stringify(welkom))
...
case 'hartatahta':
					...
					if (args.length < 1) return reply('Teks nya mana TOLOL?')
					reply('[❗] Hirti Tihti LU TOLOL :v')
